...
const Divs = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user) {
        const userName = user.name || user.employee_name || user.username || user.fullName || 'User';
  ...
  const handleDivisionSelect = async (division) => {
    ...
    try {
      ...
      console.log('Divs.jsx - Select division response status:', response.status);
      ...
      if (response.ok) {
        ...
        setError(`Failed to select division: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <div className={styles.container}>
      <div className={styles.card}>
        ...
        <p className={styles.instruction}>
          Choose your Division to continue
        </p>
