...
function StockTransferPage({ navigate }) {
 ...
 useEffect(() => {
  async function fetchInventory() {
    ...
    try {
      if (fromWarehouse) {
        const res = await axiosAPI.get(`/warehouse/${fromWarehouse.id}/inventory`);
        setFromProducts(res.data.products || []);
      }
      if (toWarehouse) {
        const res = await axiosAPI.get(`/warehouse/${toWarehouse.id}/inventory`);
  ...
  const filteredFromWarehouses = warehouses.filter(w => w.id !== toWarehouse?.id);
  const filteredToWarehouses = warehouses.filter(w => w.id !== fromWarehouse?.id);
  const availableProducts = fromProducts.filter(p => p.quantity > 0 && !selectedProductIds.includes(p.id));
  const selectedProducts = fromProducts.filter(p => selectedProductIds.includes(p.id));
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      setSuccessMessage(`Transfer successful. Transfer Number: ${res.data.transferNumber}`);
      setShowSuccess(true);
      setTransferQty({});
  ...
  return (
    <div className="container p-4">
      ...
      <h4 className="mb-4">Stock Transfer</h4>
        {loadingWarehouses ||loadingInventory && (
      ...
      <div className="row mb-4">
        <div className="col-md-5">
          ...
          <select className="form-select" value={fromWarehouse?.id || ""} onChange={(e) =>
            setFromWarehouse(warehouses.find(w => w.id === Number(e.target.value)))
          }>
            <option value="">Select From Warehouse</option>
            {filteredFromWarehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
        </div>
        ...
        <div className="col-md-5">
          ...
          <select className="form-select" value={toWarehouse?.id || ""} onChange={(e) =>
            setToWarehouse(warehouses.find(w => w.id === Number(e.target.value)))
          }>
            <option value="">Select To Warehouse</option>
            {filteredToWarehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
        </div>
      </div>
      ...
      {fromWarehouse && toWarehouse && (
        <>
          <div className="row mb-3">
            <div className="col-md-6">
              ...
              <select className="form-select" onChange={(e) => {
                ...
                if (selectedId && !selectedProductIds.includes(selectedId)) {
                    setSelectedProductIds([...selectedProductIds, selectedId]);
                }
                }}>
                <option value="">-- Select Product --</option>
                {availableProducts.map(p => {
                    const displayUnit = p.productType === "packed" ? "packets" : p.unit;
                    return (
          ...
          <div className="row">
              ...
              <table className="table table-bordered table-striped borderedtable">
                ...
                <tbody>
                  {selectedProducts.map((p, i) => {
                    ...
                    const perUnitKg = p.productType === "packed" ? convertToKg(p.packageWeight, p.unit) : convertToKg(1, p.unit);
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td>{p.name}</td>
                        <td>{p.productType}</td>
                        <td>{p.productType === "packed" ? "packets" : p.unit}</td>
                        <td>{p.productType === "packed" ? `${p.packageWeight} ${p.unit}` : "-"}</td>
                        <td>{p.quantity}</td>
                        <td>
                          <input type="number" min="0" className="form-control form-control-sm"
                            value={qty} onChange={(e) => handleQtyChange(p.id, e.target.value)} />
                        </td>
                        <td className="text-danger"> {getAdjustedQty(p.id, p.quantity, "from")}</td>
                        <td>{inTons} t</td>
                        <td>
                          <button className="btn btn-sm btn-outline-danger" onClick={() => removeProduct(p.id)}>×</button>
                        </td>
                      </tr>
                    );
                  })}
            ...
            <div className="col-md-6">
              ...
              <table className="table table-bordered table-striped borderedtable">
                ...
                <tbody>
                  {selectedProducts.map((p, i) => {
                    const toProduct = toProducts.find(tp => tp.id === p.id) || { quantity: 0 };
                    const qty = transferQty[p.id] || 0;
                    const perUnitKg = p.productType === "packed" ? convertToKg(p.packageWeight, p.unit) : convertToKg(1, p.unit);
                    const inTons = (qty * perUnitKg / 1000).toFixed(3);
                    return (
                      <tr key={p.id}>
                        ...
                        <td className="text-success">{getAdjustedQty(p.id, toProduct.quantity, "to")}</td>
                        <td>{inTons} t</td>
                      </tr>
                    );
                  })}
          ...
          <div className="mt-4 text-end">
            <button className="btn btn-primary" disabled={submitting} onClick={submitTransfer}>
